...
/** 조회의 진행 상태. 실패의 오류는 code 로 화면을 나눌 수 있도록 응답 오류 객체 그대로 둔다. */
...
type ConsoleFetchOptions = {
  /**
   * true 를 돌려주는 오류는 재조회 중이라도 보고 있던 데이터를 버리고 실패로 바꾼다.
   * effect 의 의존성이므로 렌더마다 새로 만들지 말고 모듈 수준 함수를 넘긴다.
   */
  isFatal?: (error: ConsoleApiError) => boolean;
};
 ...
 * 재조회 실패는 이미 끝난 실행의 결과를 가리지 않도록 보고 있던 데이터를 그대로 두고, isFatal 인 오류만 실패로 바꾼다.
 * fetcher 는 useCallback 으로 감싸거나 모듈 수준 함수를 넘겨야 렌더마다 조회가 반복되지 않는다.
 */
export function useConsoleFetch<T>(
  fetcher: ConsoleFetcher<T>,
  { isFatal }: ConsoleFetchOptions = {},
) {
  ...
  useEffect(() => {
    ...
    fetcher(controller.signal)
      ...
      .catch((error: unknown) => {
        ...
        setStored((previous) =>
          previous.fetcher === fetcher && previous.state.status === 'ready' && !isFatal?.(failure)
            ? previous
            : { fetcher, state: { status: 'failed', error: failure } },
        );
      });

    return () => controller.abort();
  }, [fetcher, isFatal, attempt]);
